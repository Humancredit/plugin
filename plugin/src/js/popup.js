...
(function() {
    ...
    document.querySelector('#panes > div:nth-of-type(2)').style.setProperty('height', document.querySelector('#panes > div:nth-of-type(1)').offsetHeight + 'px');
    ...
    var addFirewallRow = function(des) {
        ...
        row.toggleClass('isDomain', isDomain).toggleClass('isSubDomain', !isDomain).toggleClass('allowed', hnDetails.allowCount !== 0).toggleClass('blocked', hnDetails.blockCount !== 0).toggleClass('totalAllowed', hnDetails.totalAllowCount !== 0).toggleClass('totalBlocked', hnDetails.totalBlockCount !== 0);
    ...
    var updateFirewallCell = function(scope, des, type, rule) {
        ...
        if (matches !== null) {
            ownRule = (matches[2] !== '*' || matches[3] === type) && (matches[2] === des) && (matches[1] === scopeToSrcHostnameMap[scope]);
        }
        ...
        if (aCount !== 0 || bCount !== 0) {
            ...
            textNode.nodeValue = threePlus.slice(0, aCount) + sixSpace.slice(aCount + bCount) + threeMinus.slice(0, bCount);
        }
        else {
            textNode.nodeValue = ' ';
        }
    ...
    var updateAllFirewallCells = function() {
        ...
        for (var key in rules ) {
            ...
            updateFirewallCell(key.charAt(0), key.slice(2, key.indexOf(' ', 2)), key.slice(key.lastIndexOf(' ') + 1), rules[key]);
        }

        uDom('#firewallContainer').toggleClass('dirty', popupData.matrixIsDirty === true);
    };
    ...
    var buildAllFirewallRows = function() {
        ...
        if (dfHotspots === null) {
            dfHotspots = uDom('#actionSelector').toggleClass('colorBlind', popupData.colorBlindFriendly).on('click', 'span', setFirewallRuleHandler);
        }
        dfHotspots.detach();
        ...
        if (dfPaneBuilt !== true) {
            uDom('#firewallContainer').on('click', 'span[data-src]', unsetFirewallRuleHandler).on('mouseenter', '[data-src]', mouseenterCellHandler).on('mouseleave', '[data-src]', mouseleaveCellHandler);
            dfPaneBuilt = true;
        }
    ...
    var renderPrivacyExposure = function() {
        ...
        // of the list.
        var desHostnameDone = {};
        var keys = Object.keys(popupData.firewallRules).sort(rulekeyCompare);
        var key,
            des,
            hnDetails;
        ...
        if (allDomains.hasOwnProperty(popupData.pageDomain) === false && reNetworkRelatedURL.test(popupData.rawURL)) {
            allHostnameRows.push(popupData.pageDomain);
            allDomains[popupData.pageDomain] = false;
            allDomainCount += 1;
        }

        var summary = domainsHitStr.replace('{{count}}', touchedDomainCount.toLocaleString()).replace('{{total}}', allDomainCount.toLocaleString());
    ...
    var renderPopup = function() {
        ...
        uDom('body').toggleClass('advancedUser', popupData.advancedUserEnabled).toggleClass('off', (popupData.pageURL === '') || (!popupData.netFilteringSwitch) || (popupData.pageHostname === 'behind-the-scene' && !popupData.advancedUserEnabled));
        ...
        uDom('#firewallContainer').toggleClass('minimized', popupData.firewallPaneMinimized).toggleClass('colorBlind', popupData.colorBlindFriendly);
    ...
    var renderPopupLazy = function() {
        var onDataReady = function(data) {
            ...
            uDom('#no-cosmetic-filtering > span.badge').text( typeof v === 'number' ? v.toLocaleString() : v);
        };
    ...
    var unsetFirewallRuleHandler = function(ev) {
        var cell = uDom(this);
        setFirewallRule(cell.attr('data-src') === '/' ? '*' : popupData.pageHostname, cell.attr('data-des'), cell.attr('data-type'), 0, ev.ctrlKey || ev.metaKey);
    ...
    var setFirewallRuleHandler = function(ev) {
        ...
        setFirewallRule(cell.attr('data-src') === '/' ? '*' : popupData.pageHostname, cell.attr('data-des'), cell.attr('data-type'), action, ev.ctrlKey || ev.metaKey);
        dfHotspots.detach();
    };
    ...
    var onShowTooltip = function() {
        ...
        if (over) {
            pos = document.body.getBoundingClientRect().height - target.getBoundingClientRect().top;
    ...
    uDom.onLoad(function() {
        ...
        uDom('body').on('mouseenter', '[data-tip]', onShowTooltip).on('mouseleave', '[data-tip]', onHideTooltip);
    });
     ...
     *
     */
    var hcStartup = function() {

        // we need popup data, to access local storage, so wait til we have enough
        // infos
        if (!popupData.tabId) {
            setTimeout(hcStartup, 99);
            return;
        }

        // retrieve humancredit visibility level
        var hcLevel = vAPI.localStorage.getItem('hcLevel' + popupData.pageHostname);
        if (hcLevel === null) {
            vAPI.localStorage.getItem('hcLevel' + popupData.pageHostname, '2');
            hcLevel = 2;
        }
        var levels = uDom('input[type=radio]').nodes;
        for (var i in levels) {
            var elem = levels[i];
            if (elem.value == hcLevel) {
                elem.checked = true;
                uDom('#hcFace').addClass('level' + elem.value.toString());
                uDom('#hcFace').attr('title', elem.title);
                uDom('#hcText').text(elem.title);
            }
        };
         ...
         * user changed level
         */
        uDom('input[type=radio]').on('change', function() {

            // adjust icon
            uDom('#hcFace').removeClass('level0');
            uDom('#hcFace').removeClass('level1');
            uDom('#hcFace').removeClass('level2');
            uDom('#hcFace').removeClass('level3');
            uDom('#hcFace').addClass('level' + this.value.toString());
            uDom('#hcFace').attr('title', this.title);
            uDom('#hcText').text(this.title);

            // toggle filtering if needed
            if (!popupData || !popupData.pageURL) {
                return;
            }
            if (popupData.pageHostname === 'behind-the-scene' && !popupData.advancedUserEnabled) {
            ...
            if (this.value.toString() == '0') {
                ...
                uDom('body').removeClass('off');
            }

            // save last level
            //console.log(popupData);
            vAPI.localStorage.setItem('hcLevel' + popupData.pageHostname, this.value);
            ...
            //console.debug(popupData.tabId, this.value);

            // update popup data
            hashFromPopupData();
            setTimeout(reloadTab, 99);
        });
    };

    uDom.onLoad(hcStartup);
